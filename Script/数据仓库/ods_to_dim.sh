...
insert overwrite table ${hive_db}.dim_user_zip partition (dt)
select id,
       name,
       phone_num,
       email,
...
(
    select
        ...
        tm_id,
        create_time
    from ${hive_db}.ods_sku_info_full
    where dt='$do_date'
),
spu as
(
    select
        id,
        spu_name
    from ${hive_db}.ods_spu_info_full
    where dt='$do_date'
),
c3 as
(
    select
        ...
        name,
        category2_id
    from ${hive_db}.ods_base_category3_full
    where dt='$do_date'
),
c2 as
(
    select
        ...
        name,
        category1_id
    from ${hive_db}.ods_base_category2_full
    where dt='$do_date'
),
c1 as
(
    select
        id,
        name
    from ${hive_db}.ods_base_category1_full
    where dt='$do_date'
),
tm as
(
    select
        id,
        tm_name
    from ${hive_db}.ods_base_trademark_full
    where dt='$do_date'
),
attr as
(
    ...
    from ${hive_db}.ods_sku_attr_value_full
    where dt='$do_date'
    group by sku_id
),
sale_attr as
(
    ...
    from ${hive_db}.ods_sku_sale_attr_value_full
    where dt='$do_date'
    group by sku_id
)
insert overwrite table ${hive_db}.dim_sku_full partition(dt='$do_date')
select
    sku.id,
    sku.price,
    sku.sku_name,
...
(
    select
        id,
        region_name
    from ${hive_db}.ods_base_region_full
    where dt='$do_date'
)region
...
dim_coupon_full="
insert overwrite table ${hive_db}.dim_coupon_full partition(dt='$do_date')
select
    id,
    coupon_name,
    coupon_type,
...
(
    select
        ...
        operate_time,
        expire_time
    from ${hive_db}.ods_coupon_info_full
    where dt='$do_date'
)ci
left join
(
...
dim_activity_full="
insert overwrite table ${hive_db}.dim_activity_full partition(dt='$do_date')
select
    rule.id,
    info.id,
    activity_name,
...
(
    select
        ...
        benefit_discount,
        benefit_level
    from ${hive_db}.ods_activity_rule_full
    where dt='$do_date'
)rule
left join
(
    select
        ...
        end_time,
        create_time
    from ${hive_db}.ods_activity_info_full
    where dt='$do_date'
)info
...
select
    ...
    create_time,
    operate_time
from ${hive_db}.ods_promotion_pos_full 
where dt='$do_date';
"

dim_promotion_refer_full="
insert overwrite table ${hive_db}.dim_promotion_refer_full partition(dt='$do_date')
select
    id, 
    refer_name,
    create_time,
    operate_time
from ${hive_db}.ods_promotion_refer_full 
where dt='$do_date';
"
