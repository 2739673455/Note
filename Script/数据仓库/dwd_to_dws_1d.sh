...
insert overwrite table ${hive_db}.dws_trade_province_order_1d partition(dt='$do_date')
select
    province_id,
    province_name,
    area_code,
...
insert overwrite table ${hive_db}.dws_trade_user_cart_add_1d partition(dt='$do_date')
select
    user_id,
    count(*),
    sum(sku_num)
...
insert overwrite table ${hive_db}.dws_trade_user_order_1d partition(dt='$do_date')
select
    user_id,
...
insert overwrite table ${hive_db}.dws_trade_user_sku_order_1d partition(dt='$do_date')
select
    user_id,
    id,
    sku_name,
...
insert overwrite table ${hive_db}.dws_interaction_sku_favor_add_1d partition(dt='$do_date')
select
    sku_id,
    sku_name,
    category1_id,
...
insert overwrite table ${hive_db}.dws_tool_user_coupon_coupon_used_1d partition(dt='$do_date')
select
    user_id,
    coupon_id,
    coupon_name,
