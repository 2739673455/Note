...
insert overwrite table ${hive_db}.dwd_trade_order_detail_inc partition (dt = '$do_date')
select
    order_info.order_id,
    order_info.user_id,
    order_time,
...
from (
    ...
    from (
        ...
        having date_format(min(create_time), 'yyyy-MM-dd') = '$do_date'
         ) as play
    left join(
        select
            user_id,
...
insert overwrite table ${hive_db}.dwd_examination_test_paper_inc partition (dt ='$do_date')
select
    data.id,
    data.paper_id,
    data.user_id,
...
insert overwrite table ${hive_db}.dwd_examination_test_question_inc partition (dt ='$do_date')
select
    data.id,
    data.user_id,
    data.paper_id,
...
insert overwrite table ${hive_db}.dwd_interaction_review_inc partition (dt ='$do_date')
select
    data.id,
    data.user_id,
    data.course_id,
