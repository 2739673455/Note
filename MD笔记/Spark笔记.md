...
| coalesce() 合并分区 | rdd.coalesce(2, false)<br>默认没有shuffle操作，数据不会打乱重组<br>可能存在数据倾斜，若倾斜可以使用shuffle操作
| repartition() 重新分区 | rdd.repartition(3)<br>底层调用coalesce()
### 2.键值类型
| 参数 | 描述 |
| --- | --- |
...
| foreach() 遍历 | rdd.foreach()<br>分布式循环遍历，以分区为单位循环，可能导致结果无序
| foreachPartition() 对每个分区内遍历 | rdd.foreachPartition()