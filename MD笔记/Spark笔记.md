...
### 2.从文件创建RDD
    ...
    JavaRDD<String> rdd = jsc.textFile("data/word.txt", 3)
        #最小分区数设置为3
    Spark文件的读取和写入都是依托于MR(Hadoop)实现的，分区规则其实就是Hadoop切片规则
    Spark读取文件，采用的是Hadoop的方式读取，所以一行一行读取，跟字节数没有关系
    数据读取位置计算是以偏移量为单位来进行计算的
    分区流程:
        1. 首先获取文件的总长度 totalSize(回车，换行符也计算在内)
        2. 计算平均长度 goalSize = totalSize / numSplits
        3. 获取块大小 blockSize = 128M
        4. 计算切分大小 splitSize = Math.max(minSize, Math.min(goalSize, blockSize));
            文件总大小 / 填写的分区数，和块大小比较，谁小拿谁进行切分
        5. 使用splitSize按照1.1倍原则切分整个文件，如果最后剩余数据大小大于 0.1*splitSize，再加一个分区
        6. 如果切分的位置位于一行的中间，会在当前分区读完一整行数据
    例如:
